...
public class TuplaBinaria {
	...
	public void set(int cardinal, int ID) {
		...
		numBits = cardinal;
		// zizeOfUnidad = sizeof(unsigned int) * 8; // - registro EAX
	...
	//***************************************************************************
	public void mascComp(TuplaBinaria mascara, TuplaBinaria x, TuplaBinaria mascaraAcep) {
	  int i;
	  //unsigned int temp1, temp2, temp3;

	  for (i=0; i<numUnidades; i++) {
	    // - Propuesta inicial Lias.	  
	   /* unidad[i] = (((mascara.unidad[i] ^ x->unidad[i]) & mascara.unidad[i]) |
	                (~mascaraAcep.unidad[i] & x->unidad[i]));   */

	     // - Cambio Propuesto Yosvany (autor principal del LEX). OK.
	...
	//   1. Mirar las tuplas como los números que representan en notación binaria.
	//   2. Mirar las tuplas según la cantidad de elementos unitarios que contengan.
	//***************************************************************************
	public boolean mayorQue(TuplaBinaria operando, int opc) {
	  int i;
	  if (opc == 2) return cantValoresUnitarios() > operando.cantValoresUnitarios();
	  for (i = numUnidades-1; i>=0; i-- )
	    if (unidad[i] != operando.unidad[i])
	      if (unidad[i] > operando.unidad[i]) return true;
	      else return false;
	  return false;
	}
	//---------------------------------------------------------------------------

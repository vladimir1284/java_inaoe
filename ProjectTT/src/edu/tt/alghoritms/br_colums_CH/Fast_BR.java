//----------------------------------------------------------------------------
// - modofocacion del BR: 
//   Reduccion del número de columnas, generar los pseudo TT,
//   reconstruccion de los TT originales.
//----------------------------------------------------------------------------
package edu.tt.alghoritms.br_colums_CH;

import edu.tt.InterfaceTT;
import edu.tt.Output;

public class Fast_BR implements InterfaceTT {
	...
	public int ordenInicial() {
	  ...
	  //---------------
	  rotar(DERECHA);
	  quickSort(2, -1, -1);    // - Ordeno las fila de mayor a menor según la cantidad de 1.
	  rotar(IZQUIERDA);
	  k = ordenarFilas();
	  //quickSort(2, 0, k-1);
	...
	//---------------------------------------------------------------------------
	//***************************************************************************
	// - Ordena mediante el método QuickSort de mayor a menor.
	//  En dependencia del parametro opc:
	//  Si opc = 1 -> Ordena los rasgos por su valor # como notación binaria
	//	   opc = 2 -> Ordena los rasgos según la cantidad de valores unitarios.
	// - También brinda la posibilidad de ordenar un rango de rasgos determinado
	//   por los parámetros izq y der.
	//***************************************************************************
	...
	/*  Nombre Procedimiento: OrdRap
	 Parámetros:
	    ...
	    índice inferior (indiceIzq)
	    índice superior (indiceDer)*/
	...
	//---------------------------------------------------------------------------
	//***************************************************************************
	// - Las listas de tl estarán representadas por las filas de la matriz pMatriz,
	//  y la última lista de tl estará siempre indicada por el índice listaIndex.
	// - Los índices (primero y ultimo) indexan los rasgos iniciales y finales
	...
	public void extraerTT(Output registerData) {
	  ...
	  registerData.getInfo("\n   ALGORITMO FAST BR - BR CON REDUCCION DE COLUMNAS Y HUECOS");
	  registerData.getInfo("\n ********************************************\n");
	 ...
	 //******************************************    Inicializacion
      ...
      while (true) {		  
		  ...
		  for (j=primero+1; j<=ultimo; j++) {  //- para cada elem. enCurso_x de prox(lu)
		    ...
		    // ************* - PROPOCICIÓN  2.4 - **********************
		      seAcepta = false;    // - Si no se cumple la condición #1
		      if (mascaraAcep[listaIndex+1].mascAcep(mascaraAcep[listaIndex], enCurso_x) == false) {
		        ...
		        if(condicion2 == false) {
		          seAcepta = true;          // - PROPOSICIÓN 2.5 - Condicion #3.
		        ...
		        }       //-Artículo 176-177 - Reglamento docente..
		      }
		    // *********************************************************
		  ...
		  if (primero+1 < ultimo) { // -> Si  | l'| > 1
		    ...
		    listaIndex++;                   // - Se indexa la próxima lista.
		   ...
		   // - Indexo y extraigo intervalo, de la lista anterior.
		  primero = pilaPrimero.pop();   //-No sería de la anterior sinó...
...
*/	}
	...
	//---------------------------------------------------------------------------
	public void operatorIgual(Fast_BR objeto) {
	  int i, j;
